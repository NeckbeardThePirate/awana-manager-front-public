export default async function (clubberId: string, bookName: string, bookId: string,  getToken: () => Promise<string | null>) {
	try {
		...
		const url = import.meta.env.VITE_API_URL + '/update-clubber-book';
